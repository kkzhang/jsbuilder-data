...
ccs.dataReaderHelper = {ConfigType: {DragonBone_XML: 0, CocoStudio_JSON: 1, CocoStudio_Binary: 2}, _configFileList: [], _flashToolVersion: ccs.CONST_VERSION_2_0, _positionReadScale: 1, _asyncRefCount: 0, _asyncRefTotalCount: 0, _dataQueue: null, setPositionReadScale: function (scale) {
    ...
    if (this._configFileList.indexOf(filePath) != -1) {
        if (target && selector)if (this._asyncRefTotalCount == 0 && this._asyncRefCount == 0)this._asyncCallBack(selector, target, 1); else this._asyncCallBack(selector, target, (this._asyncRefTotalCount - this._asyncRefCount) / this._asyncRefTotalCount);
        return
    }
    ...
    for (var i = 0; i < len; i++)if (locFileList[i] == configFile)it = i;
    ...
    var armaturesXML = skeleton.querySelectorAll(ccs.CONST_SKELETON +
    ...
    decodeArmatureFromJSON: function (json, dataInfo) {
        ...
        var isArmature = parseFloat(displayXML.getAttribute(ccs.CONST_A_IS_ARMATURE)) ||
            0;
        ...
        switch (displayType) {
            case ccs.DISPLAY_TYPE_SPRITE:
                displayData =
                    new ccs.SpriteDisplayData;
                var name = json[ccs.CONST_A_NAME];
                if (name != null)displayData.displayName = name;
            ...
            case ccs.DISPLAY_TYPE_ARMATURE:
                displayData = new ccs.ArmatureDisplayData;
                var name = json[ccs.CONST_A_NAME];
                if (name != null)displayData.displayName = json[ccs.CONST_A_NAME];
                break;
            case ccs.DISPLAY_TYPE_PARTICLE:
                displayData = new ccs.ParticleDisplayData;
                var plist = json[ccs.CONST_A_PLIST];
                if (plist != null)if (dataInfo.asyncStruct)displayData.displayName =
                    dataInfo.asyncStruct.basefilePath + plist; else displayData.displayName = dataInfo.basefilePath + plist;
                break;
            default:
                displayData = new ccs.SpriteDisplayData;
                break
        ...
        aniData.name = name;
        var movementsXML = animationXML.querySelectorAll(ccs.CONST_ANIMATION + " \x3e " +
            ccs.CONST_MOVEMENT);
        ...
        var aniData = new ccs.AnimationData;
        var name = json[ccs.CONST_A_NAME];
        if (name)aniData.name = json[ccs.CONST_A_NAME];
        var movementDataList = json[ccs.CONST_MOVEMENT_DATA] || [];
        for (var i = 0; i < movementDataList.length; i++) {
            var locMovementData =
                this.decodeMovementFromJson(movementDataList[i], dataInfo);
        ...
        if (easing)if (easing != ccs.CONST_FL_NAN) {
            ...
            movementData.tweenEasing = tweenEasing == 2 ? ccs.TweenType.sineEaseInOut :
                tweenEasing
        ...
        movementData.durationTo = json[ccs.CONST_A_DURATION_TO] ||
            0;
        movementData.duration = json[ccs.CONST_A_DURATION] || 0;
        if (json[ccs.CONST_A_DURATION] == null)movementData.scale = 1; else movementData.scale = json[ccs.CONST_A_MOVEMENT_SCALE] == null ? 1 : json[ccs.CONST_A_MOVEMENT_SCALE];
        ...
        for (var i = 0; i < movementBoneList.length; i++) {
            var locMovementBoneData =
                this.decodeMovementBoneFromJson(movementBoneList[i], dataInfo);
        ...
        for (var ii = 0; ii < framesXML.length; ii++) {
            var frameXML =
                framesXML[ii];
            if (parentXml)while (j < length && (parentFrameXML ? totalDuration < parentTotalDuration || totalDuration >= parentTotalDuration + currentDuration : true)) {
        ...
        frameData.copy(movBoneData.frameList[movBoneData.frameList.length -
            1]);
        ...
        for (var i = 0; i < length; i++) {
            ...
            var frameData = this.decodeFrameFromJson(dic,
                dataInfo);
        ...
        if (dataInfo.cocoStudioVersion < ccs.VERSION_CHANGE_ROTATION_RANGE) {
            ...
            for (var i = frames.length - 1; i >= 0; i--)if (i > 0) {
                var difSkewX = frames[i].skewX - frames[i - 1].skewX;
                var difSkewY = frames[i].skewY - frames[i - 1].skewY;
                if (difSkewX < -pi || difSkewX > pi)frames[i - 1].skewX =
                        difSkewX < 0 ? frames[i - 1].skewX - 2 * pi : frames[i - 1].skewX + 2 * pi;
                if (difSkewY < -pi || difSkewY > pi)frames[i - 1].skewY = difSkewY < 0 ? frames[i - 1].skewY - 2 * pi : frames[i - 1].skewY + 2 * pi
            }
        }
        ...
        if (zOrder != null)frameData.zOrder =
            parseInt(zOrder);
        ...
        if (_easing != null)if (_easing != ccs.CONST_FL_NAN) {
            tweenEasing = frameXML.getAttribute(ccs.CONST_A_TWEEN_EASING);
            if (tweenEasing)frameData.tweenEasing = tweenEasing == 2 ? ccs.TweenType.sineEaseInOut : tweenEasing
        ...
        var twEPs = json[ccs.CONST_A_EASING_PARAM] ||
            [];
        for (var i = 0; i < twEPs.length; i++)frameData.easingParams[i] = twEPs[i];
        return frameData
        ...
        if (dataInfo.flashToolVersion >= ccs.CONST_VERSION_2_0) {
            ...
            px =
                parseFloat(textureXML.getAttribute(ccs.CONST_A_PIVOT_X)) || 0;
        ...
        for (var i = 0; i < contoursXML.length; i++)textureData.addContourData(this.decodeContour(contoursXML[i],
            dataInfo));
        return textureData
        ...
        for (var i = 0; i < contourDataList.length; i++)textureData.contourDataList.push(this.decodeContourFromJson(contourDataList[i]));
        return textureData
    ...
    decodeContourFromJson: function (json) {
        ...
        for (i = 0; i < animationDataArr.length; i++) {
            ...
            ccs.armatureDataManager.addAnimationData(animationData.name, animationData,
                dataInfo.filename)
        }
        ...
        node.y =
            json[ccs.CONST_A_Y] * this._positionReadScale;
        ...
        if (dataInfo.cocoStudioVersion < ccs.VERSION_COLOR_READING) {
            colorDic = json[0];
            if (colorDic) {
                node.a = colorDic[ccs.CONST_A_ALPHA] == null ? 255 :
                    colorDic[ccs.CONST_A_ALPHA];
            ...
            if (colorDic) {
                ...
                node.g = colorDic[ccs.CONST_A_GREEN] ==
                    null ? 255 : colorDic[ccs.CONST_A_GREEN];
                node.b = colorDic[ccs.CONST_A_BLUE] == null ? 255 : colorDic[ccs.CONST_A_BLUE];
                node.isUseColorInfo = true
            }
        }
        ...
        if (selector && typeof selector === "function")selector.call(target, percent);
...
ccs.TransformHelp.transformFromParentWithoutScale = function (node, parentNode) {
    for (var p in parentNode)this.helpParentNode[p] = parentNode[p];
...
ccs.TweenFunction.tweenTo = function (time, type, easingParam) {
    ...
    switch (type) {
        ...
        case ccs.TweenType.elasticEaseIn:
            var period = 0.3;
            if (null != easingParam)period = easingParam[0];
...
ccs.TweenFunction.elasticEaseInOut = function (time, easingParam) {
    ...
    if (time == 0 || time == 1)newT = time; else {
        ...
        var s = period / 4;
        time = time - 1;
        if (time < 0)newT = -0.5 * Math.pow(2, 10 * time) * Math.sin((time - s) * ccs.DOUBLE_PI / period); else newT = Math.pow(2, -10 * time) * Math.sin((time - s) * ccs.DOUBLE_PI / period) * 0.5 + 1
    }
    return newT
};
...
ccs.TweenFunction.easeInOut = function (time, rate) {
    time *= 2;
    if (time < 1)return 0.5 * Math.pow(time, rate); else return 1 - 0.5 * Math.pow(2 - time, rate)
};
...
ccs.armatureDataManager = {_animationDatas: {}, _armarureDatas: {}, _textureDatas: {}, _autoLoadSpriteFile: false, _relativeDatas: {}, s_sharedArmatureDataManager: null, removeArmatureFileInfo: function (configFilePath) {
    ...
    var data = this.getRelativeData(configFilePath);
    if (data)data.armatures.push(id);
    this._armarureDatas[id] = armatureData
}, getArmatureData: function (id) {
    var armatureData = null;
    if (this._armarureDatas)armatureData =
        this._armarureDatas[id];
    return armatureData
    ...
    if (this._animationDatas[id])animationData = this._animationDatas[id];
...
ccs.FrameData = ccs.BaseData.extend({duration: 0, tweenEasing: 0, easingParamNumber: 0, easingParams: null, displayIndex: -1, movement: "", event: "", sound: "", soundEffect: "", blendFunc: 0, frameID: 0, isTween: true, ctor: function () {
    ...
    if (this.easingParamNumber != 0)for (var i = 0; i < this.easingParamNumber; i++)this.easingParams[i] = frameData.easingParams[i];
...
ccs.displayFactory = {addDisplay: function (bone, decoDisplay, displayData) {
    ...
    switch (bone.getDisplayRenderNodeType()) {
        case ccs.DISPLAY_TYPE_SPRITE:
            ...
            break;
        case ccs.DISPLAY_TYPE_PARTICLE:
            this.updateParticleDisplay(bone,
                display, dt);
            break;
        case ccs.DISPLAY_TYPE_ARMATURE:
    ...
    if (ccs.ENABLE_PHYSICS_CHIPMUNK_DETECT || ccs.ENABLE_PHYSICS_SAVE_CALCULATED_VERTEX)if (dirty) {
    ...
    addSpriteDisplay: function (bone, decoDisplay, displayData) {
        ...
        var startPos = textureName.lastIndexOf(".");
        if (startPos != -1)textureName = textureName.substring(0, startPos);
        if (textureName == "")skin = ccs.Skin.create(); else skin = ccs.Skin.createWithSpriteFrameName(textureName +
            ".png");
        ...
        if (armature)if (armature.getArmatureData().dataVersion >= ccs.CONST_VERSION_COMBINED)skin.setSkinData(displayData.skinData); else skin.setSkinData(bone.boneData)
    }, initSpriteDisplay: function (bone, decoDisplay, displayName, skin) {
        var textureName = displayName;
        var startPos = textureName.lastIndexOf(".");
        if (startPos != -1)textureName = textureName.substring(0,
            startPos);
        var textureData = ccs.armatureDataManager.getTextureData(textureName);
        if (textureData)skin.setAnchorPoint(cc.p(textureData.pivotX, textureData.pivotY));
...
ccs.DisplayManager = ccs.Class.extend({_decoDisplayList: null, _currentDecoDisplay: null, _displayRenderNode: null, _displayIndex: null, _forceChangeDisplay: false, _bone: null, _visible: true, _displayType: null, ctor: function () {
    ...
    changeDisplayWithIndex: function (index, force) {
        ...
        for (var i = 0; i < locDisplayList.length; i++)if (locDisplayList[i].getDisplayData().displayName == name) {
        ...
        if (displayRenderNode) {
            ...
            } else if (displayRenderNode instanceof cc.ParticleSystem)if (displayRenderNode instanceof ccs.Armature) {
    ...
    initDisplayList: function (boneData) {
        ...
        if (!this._displayRenderNode)return cc.size(0,
            0);
...
ccs.Skin = ccs.Sprite.extend({_skinData: null, bone: null, _skinTransform: null, _displayName: "", _armature: null, _className: "Skin", ctor: function () {
    ...
    this._transform =
        cc.affineTransformConcat(this._skinTransform, this.bone.getNodeToArmatureTransform())
}, _updateTransformForWebGL: function () {
    var locQuad = this._quad;
    if (!this._visible)locQuad.br.vertices = locQuad.tl.vertices = locQuad.tr.vertices = locQuad.bl.vertices = {x: 0, y: 0, z: 0}; else {
    ...
    if (this._textureAtlas)this._textureAtlas.updateQuad(locQuad, this._textureAtlas.getTotalQuads());
    ...
    if (!cc.SPRITEBATCHNODE_RENDER_SUBPIXEL)return Math.ceil(__ARGS__); else return __ARGS__
}, getNodeToWorldTransform: function () {
    return cc.affineTransformConcat(this._transform,
        this.bone.getArmature().getNodeToWorldTransform())
...
if (cc._renderType == cc._RENDER_TYPE_WEBGL)ccs.Skin.prototype.updateTransform = ccs.Skin.prototype._updateTransformForWebGL; else;
...
ccs.Skin.create = function (fileName, rect) {
    ...
    if (argnum === 0 || fileName == null || fileName == "") {
        if (skin.init())return skin
    } else if (fileName[0] == "#") {
        if (skin && skin.initWithSpriteFrameName(fileName))return skin
    } else if (skin && skin.initWithFile(fileName, rect))return skin;
    return null
};
ccs.Skin.createWithSpriteFrameName = function (spriteFrameName) {
    var skin = new ccs.Skin;
    if (skin && skin.initWithSpriteFrameName(spriteFrameName))return skin;
    return null
};
...
ccs.ArmatureAnimation = ccs.ProcessBase.extend({_animationData: null, _movementData: null, _armature: null, _movementID: "", _toIndex: 0, _tweenList: null, _speedScale: 1, _ignoreFrameEvent: false, _frameEventQueue: null, _movementEventQueue: null, _movementList: null, _onMovementList: false, _movementListLoop: false, _movementIndex: 0, _movementListDurationTo: -1, _movementEventCallFunc: null, _frameEventCallFunc: null, _movementEventTarget: null, _frameEventTarget: null, _movementEventListener: null, _frameEventListener: null, ctor: function () {
    ...
    for (var i = 0; i < locTweenList.length; i++)locTweenList[i].pause();
    ccs.ProcessBase.prototype.pause.call(this)
}, resume: function () {
    var locTweenList = this._tweenList;
    for (var i = 0; i < locTweenList.length; i++)locTweenList[i].resume();
    ccs.ProcessBase.prototype.resume.call(this)
}, stop: function () {
    var locTweenList =
        this._tweenList;
    for (var i = 0; i < locTweenList.length; i++)locTweenList[i].stop();
    ...
    durationTo =
            durationTo === undefined ? -1 : durationTo;
    ...
    ccs.ProcessBase.prototype.play.call(this,
    ...
    for (var element in map) {
        ...
        if (movementBoneData && movementBoneData.frameList.length >
            0) {
            ...
            if (bone.getChildArmature())bone.getChildArmature().getAnimation().setProcessScale(this._processScale)
        } else if (!bone.isIgnoreMovementBoneData()) {
    ...
    var locTweenList = this._tweenList;
    for (var i = 0; i < locTweenList.length; i++)locTweenList[i].gotoAndPlay(frameIndex);
    this._armature.update(0);
    ...
    update: function (dt) {
        ...
        var locTweenList = this._tweenList;
        for (var i = 0; i < locTweenList.length; i++)locTweenList[i].update(dt);
        ...
        while (frameEvents.length > 0) {
            ...
            if (this._frameEventListener)this._frameEventListener(event.bone,
                event.frameEventName, event.originFrameIndex, event.currentFrameIndex);
        ...
        if (arguments.length == 1)this._frameEventListener = target; else if (arguments.length == 2) {
        ...
        if (this._onMovementList) {
            ...
            } else if (this._movementIndex < locMovementList.length) {
                movementObj = locMovementList[this._movementIndex];
                this.play(movementObj, movementObj.durationTo, 0);
                this._movementIndex++
...
ccs.Tween = ccs.ProcessBase.extend({_tweenData: null, _to: null, _from: null, _between: null, _movementBoneData: null, _bone: null, _frameTweenEasing: 0, _betweenDuration: 0, _totalDuration: 0, _toIndex: 0, _fromIndex: 0, _animation: null, _passLastFrame: false, ctor: function () {
    ccs.ProcessBase.prototype.ctor.call(this);
    ...
    if (this._rawDuration == 0) {
        ...
        if (durationTo == 0)this.setBetween(nextKeyFrame, nextKeyFrame); else this.setBetween(this._tweenData,
            nextKeyFrame);
        this._frameTweenEasing = ccs.TweenType.linear
    } else if (this._movementBoneData.frameList.length > 1) {
        this._durationTween = durationTween * this._movementBoneData.scale;
        if (loop && this._movementBoneData.delay != 0)this.setBetween(this._tweenData, this.tweenNodeTo(this.updateFrameData(1 - this._movementBoneData.delay), this._between)); else if (!difMovement || durationTo == 0)this.setBetween(nextKeyFrame, nextKeyFrame); else this.setBetween(this._tweenData, nextKeyFrame)
    }
    this.tweenNodeTo(0)
}, gotoAndPlay: function (frameIndex) {
...
ccs.ColliderDetector = ccs.Class.extend({_colliderBodyList: null, _bone: null, _body: null, _active: false, _filter: null, helpPoint: cc.p(0, 0), ctor: function () {
    ...
    for (i = 0; i < locBodyList.length; i++) {
        ...
        if (body && body.getContourData() ==
            contourData)eraseList.push(body)
    }
    for (i = 0; i < eraseList.length; i++)cc.arrayRemoveObject(locBodyList, eraseList[i])
}, removeAll: function () {
    this._colliderBodyList.length = 0
    ...
    if (locBody) {
        ...
        if (this._active)for (var i = 0; i < this._colliderBodyList.length; i++) {
            ...
            this._colliderBodyList.length; i++) {
            colliderBody = this._colliderBodyList[i];
            locShape = colliderBody.getShape();
...
ccs.Armature = ccs.Node.extend({animation: null, armatureData: null, batchNode: null, _textureAtlas: null, _parentBone: null, _boneDic: null, _topBoneList: null, _armatureIndexDic: null, _offsetPoint: null, version: 0, _armatureTransformDirty: true, _body: null, _blendFunc: null, _className: "Armature", _realAnchorPointInPoints: null, ctor: function (name, parentBone) {
    ...
    changeBoneParent: function (bone, parentName) {
        ...
        if (ax != locAnchorPoint.x || ay != locAnchorPoint.y) {
            ...
            this._anchorPointInPoints.x =
                contentSize.width * locAnchorPoint.x - this._offsetPoint.x;
        ...
        this._realAnchorPointInPoints.x = this._contentSize.width *
            x;
        this.setNodeDirty()
        ...
        for (var i = 0, len = locChildren.length; i < len; i++) {
            ...
            if (selBone) {
                ...
                switch (selBone.getDisplayRenderNodeType()) {
                    case ccs.DISPLAY_TYPE_SPRITE:
                        if (node instanceof ccs.Skin)if (cc._renderType === cc._RENDER_TYPE_WEBGL) {
                            node.updateTransform();
                            var func = selBone.getBlendFunc();
                            if (func.src != alphaPremultiplied.src || func.dst != alphaPremultiplied.dst)node.setBlendFunc(selBone.getBlendFunc()); else if (this._blendFunc.src == alphaPremultiplied.src && this._blendFunc.dst == alphaPremultiplied.dst && !node.getTexture().hasPremultipliedAlpha())node.setBlendFunc(alphaNonPremultipled);
                            else node.setBlendFunc(this._blendFunc);
                            node.draw(ctx)
                        } else node.visit(ctx);
                        break;
                    case ccs.DISPLAY_TYPE_ARMATURE:
            ...
            } else if (selBone instanceof cc.Node)selBone.visit(ctx)
        }
    }, onEnter: function () {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (bone) {
                ...
                if (first) {
                    ...
                    maxX = r.x + r.width > boundingBox.x + boundingBox.width ? r.x + r.width : boundingBox.x +
                        boundingBox.width;
    ...
    setParentBone: function (parentBone) {
        ...
        var locBoneDic = this._boneDic;
        for (var key in locBoneDic)locBoneDic[key].setArmature(this)
...
if (cc._renderType == cc._RENDER_TYPE_WEBGL)ccs.Armature.prototype.visit = ccs.Armature.prototype._visitForWebGL; else ccs.Armature.prototype.visit = ccs.Armature.prototype._visitForCanvas;
...
ccs.Armature.create = function (name, parentBone) {
    ...
    if (armature.init(name, parentBone))return armature;
    return null
};
ccs.Bone = ccs.Node.extend({_boneData: null, _armature: null, _childArmature: null, _displayManager: null, ignoreMovementBoneData: false, _tween: null, _tweenData: null, _parentBone: null, _boneTransformDirty: false, _worldTransform: null, _blendFunc: 0, blendDirty: false, _worldInfo: null, _armatureParentBone: null, _dataVersion: 0, _className: "Bone", ctor: function () {
    cc.Node.prototype.ctor.call(this);
    ...
    if (this._boneTransformDirty) {
        ...
        if (this._dataVersion >= ccs.CONST_VERSION_COMBINED) {
            ccs.TransformHelp.nodeConcat(locTweenData,
                this._boneData);
        ...
        if (this._parentBone)this.applyParentTransform(this._parentBone);
        else if (this._armatureParentBone)this.applyParentTransform(this._armatureParentBone);
        ccs.TransformHelp.nodeToMatrix(locWorldInfo, this._worldTransform);
        if (this._armatureParentBone)this._worldTransform = cc.affineTransformConcat(this._worldTransform, this._armature.getNodeToParentTransform())
    }
    ...
    locWorldInfo.scaleY = locWorldInfo.scaleY *
        locParentWorldInfo.scaleY;
    ...
    if (display != null) {
        display.setColor(cc.color(this._displayedColor.r * this._tweenData.r / 255, this._displayedColor.g * this._tweenData.g / 255, this._displayedColor.b * this._tweenData.b / 255));
    ...
    if (this._childArmature != armature) {
        if (armature ==
            null && this._childArmature)this._childArmature.setParentBone(null);
    ...
    getDisplayRenderNode: function () {
        ...
        this.changeDisplayWithIndex(index,
            force)
        ...
        var displayList =
            this._displayManager.getDecorativeDisplayList();
        for (var i = 0; i < displayList.length; i++) {
...
ccs.objectFactory = {_typeMap: {}, destroyInstance: function () {
    ...
    if (t)if (typeof t._fun == "function")o = new t._fun; else o = t._fun;
    return o
...
ccs.CheckBoxReader = {getInstance: function () {
    ...
    var frontCrossFileName =
        ccs.WidgetReader.getResourcePath(frontCrossDic, "path", frontCrossType);
    ...
    if (options["selectedState"])checkBox.setSelectedState(options["selectedState"]);
    ccs.WidgetReader.setColorPropsFromJsonDictionary.call(this, widget,
        options)
}};
...
ccs.LabelReader = {getInstance: function () {
    ...
    var fs = options["fontSize"];
    if (fs != null)label.setFontSize(options["fontSize"]);
    var fn = options["fontName"];
    if (fn != null)label.setFontName(options["fontName"]);
    ...
    if (ha != null)label.setTextHorizontalAlignment(options["hAlignment"]);
    var va = options["vAlignment"];
    if (va != null)label.setTextVerticalAlignment(options["vAlignment"]);
    ccs.WidgetReader.setColorPropsFromJsonDictionary.call(this, widget, options)
}};
...
ccs.WidgetReader = {getInstance: function () {
    ...
    if (isAnchorPointXExists !=
        null || isAnchorPointYExists != null)widget.setAnchorPoint(cc.p(anchorPointXInFile, anchorPointYInFile))
}, getResourcePath: function (dict, key, texType) {
    var imageFileName = dict[key];
    var imageFileName_tp;
    if (null != imageFileName)if (texType == 0)imageFileName_tp = ccs.uiReader.getFilePath() + imageFileName; else if (texType == 1)imageFileName_tp = imageFileName; else cc.assert(0, "invalid TextureResType!!!");
...
(function () {
    ...
    factoryCreate.registerType({_className: "TextAtlasReader",
    ...
    factoryCreate.registerType({_className: "ScrollView",
...
ccs.uiReader = {_filePath: "", _olderVersion: false, _fileDesignSizes: {}, _mapObject: {}, _mapParseSelector: {}, getVersionInteger: function (str) {
    ...
    var ite = parseInt(te);
    var is = parseInt(s);
    return it * 1E3 + ih * 100 + ite * 10 + is
    ...
    if (!jsonDict)throw"Please load the resource first : " +
        fileName;
...
ccs.WidgetPropertiesReader = ccs.Class.extend({_filePath: "", createWidget: function (jsonDict, fullPath, fileName) {
    ...
    if (name == "Panel")convertedClassName = "Layout"; else if (name == "TextArea")convertedClassName = "Text"; else if (name == "TextButton")convertedClassName = "Button"; else if (name == "Label")convertedClassName =
        "Text"; else if (name == "LabelAtlas")convertedClassName = "TextAtlas"; else if (name == "LabelBMFont")convertedClassName = "TextBMFont";
    return convertedClassName
}, getWidgetReaderClassName: function (classname) {
    var readerName = classname;
    if (readerName == "Panel")readerName = "Layout"; else if (readerName == "TextArea")readerName = "Text"; else if (readerName == "TextButton")readerName = "Button"; else if (readerName == "Label")readerName = "Text"; else if (readerName == "LabelAtlas")readerName = "TextAtlas"; else if (readerName == "LabelBMFont")readerName =
        "TextBMFont";
    readerName += "Reader";
    return readerName
}, getWidgetReaderClassNameFromWidget: function (widget) {
    var readerName;
    if (widget instanceof ccui.Button)readerName = "ButtonReader"; else if (widget instanceof ccui.CheckBox)readerName = "CheckBoxReader"; else if (widget instanceof ccui.ImageView)readerName = "ImageViewReader"; else if (widget instanceof ccui.TextAtlas)readerName = "TextAtlasReader"; else if (widget instanceof ccui.TextBMFont)readerName = "TextBMFontReader"; else if (widget instanceof ccui.Text)readerName =
        "TextReader"; else if (widget instanceof ccui.LoadingBar)readerName = "LoadingBarReader"; else if (widget instanceof ccui.Slider)readerName = "SliderReader"; else if (widget instanceof ccui.TextField)readerName = "TextFieldReader"; else if (widget instanceof ccui.ListView)readerName = "ListViewReader"; else if (widget instanceof ccui.PageView)readerName = "PageViewReader"; else if (widget instanceof ccui.ScrollView)readerName = "ScrollViewReader"; else if (widget instanceof ccui.Layout)readerName = "LayoutReader"; else if (widget instanceof
        ccui.Widget)readerName = "WidgetReader";
    return readerName
...
ccs.WidgetPropertiesReader0250 = ccs.WidgetPropertiesReader.extend({createWidget: function (jsonDict, fullPath, fileName) {
    ...
    textArea.setColor(cc.color(cr == null ? 255 : cr, cg == null ? 255 : cg,
            cb == null ? 255 : cb));
...
ccs.WidgetPropertiesReader0300 = ccs.WidgetPropertiesReader.extend({createWidget: function (jsonDict, fullPath, fileName) {
    ...
    if (object && selector)selector(classType,
    ...
    if (reader)this.setPropsForAllWidgetFromJsonDictionary(reader, widget, uiOptions); else {
        ...
        if (reader && widget) {
            this.setPropsForAllWidgetFromJsonDictionary(reader,
                widget, uiOptions);
    ...
    for (var i = 0; i < childrenItem.length; i++) {
        ...
        if (child)if (widget instanceof ccui.PageView)widget.addPage(child); else if (widget instanceof
            ccui.ListView)widget.pushBackCustomItem(child); else {
    ...
    if (options["opacity"] !== undefined)widget.setOpacity(options["opacity"]);
    var colorR = options["colorR"] !== undefined ?
        options["colorR"] : 255;
    var colorG = options["colorG"] !== undefined ? options["colorG"] : 255;
    var colorB = options["colorB"] !== undefined ? options["colorB"] : 255;
    widget.setColor(cc.color(colorR, colorG, colorB));
    ...
    var colorType = options["colorType"];
    panel.setBackGroundColorType(colorType);
    panel.setBackGroundColor(cc.color(scr, scg,
        scb), cc.color(ecr, ecg, ecb));
    panel.setBackGroundColor(cc.color(cr, cg, cb));
    panel.setBackGroundColorOpacity(co);
    var imageFileNameDic = options["backGroundImageData"] || {};
    var imageFileNameType = imageFileNameDic["resourceType"];
    ...
    this.setPropsForWidgetFromJsonDictionary(widget, options);
    var slider = widget;
    var barTextureScale9Enable =
        options["barTextureScale9Enable"] || false;
    slider.setScale9Enabled(barTextureScale9Enable);
    var barLength = options["length"];
    var bt = options["barFileName"] !== undefined;
    if (bt)if (barTextureScale9Enable) {
    ...
    var cri = options["textColorR"] !== undefined ? options["textColorR"] : 255;
    var cgi = options["textColorG"] !== undefined ? options["textColorG"] : 255;
    var cbi = options["textColorB"] !==
        undefined ? options["textColorB"] : 255;
    textButton.setTitleColor(cc.color(cri, cgi, cbi));
    if (options["fontSize"] !== undefined)textButton.setTitleFontSize(options["fontSize"]);
    if (options["fontName"] !== undefined)textButton.setTitleFontName(options["fontName"]);
    this.setColorPropsForWidgetFromJsonDictionary(widget, options)
    ...
    this.setPropsForLayoutFromJsonDictionary(widget,
        options);
