...
cc._tmp.WebGLSprite = function () {
    ...
    _p.updateTransform = function () {
        ...
        if (cc.SPRITE_DEBUG_DRAW) {
            var vertices = [cc.p(_t._quad.bl.vertices.x,
                _t._quad.bl.vertices.y), cc.p(_t._quad.br.vertices.x, _t._quad.br.vertices.y), cc.p(_t._quad.tr.vertices.x, _t._quad.tr.vertices.y), cc.p(_t._quad.tl.vertices.x, _t._quad.tl.vertices.y)];
    ...
    _p.addChild = function (child, localZOrder, tag) {
        ...
        if (_t._batchNode) {
            ...
            if (child.texture._webTextureObj !==
                _t.textureAtlas.texture._webTextureObj)cc.log(cc._LogInfos.Sprite_addChild_2);
    ...
    _p.isFrameDisplayed = function (frame) {
        return cc.rectEqualToRect(frame.getRect(),
            this._rect) && frame.getTexture().getName() == this._texture.getName() && cc.pointEqualToPoint(frame.getOffset(), this._unflippedOffsetPositionFromCenter)
    };
    ...
    _p.setTexture = function (texture) {
        ...
        cc.assert(!texture ||
            texture instanceof cc.Texture2D, cc._LogInfos.Sprite_setTexture_2);
    ...
    _p.draw = function () {
        var _t = this;
        if (!_t._textureLoaded)return;
        var gl = cc._renderContext, locTexture =
            _t._texture;
        if (locTexture) {
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, _t._quadWebBuffer);
            if (_t._quadDirty) {
                cc._renderContext.bufferData(cc._renderContext.ARRAY_BUFFER,
                    _t._quad.arrayBuffer, cc._renderContext.STATIC_DRAW);
                _t._quadDirty = false
            }
            gl.vertexAttribPointer(cc.VERTEX_ATTRIB_POSITION, 3, gl.FLOAT, false, 24, 0);
        ...
        if (cc.SPRITE_DEBUG_DRAW === 1 || _t._showNode) {
            var locQuad = _t._quad;
            var verticesG1 = [cc.p(locQuad.tl.vertices.x, locQuad.tl.vertices.y), cc.p(locQuad.bl.vertices.x, locQuad.bl.vertices.y),
                cc.p(locQuad.br.vertices.x, locQuad.br.vertices.y), cc.p(locQuad.tr.vertices.x, locQuad.tr.vertices.y)];
            ...
            cc._drawingUtil.drawPoly(verticesG2,
                4, true)
        }
    };
    delete _p
