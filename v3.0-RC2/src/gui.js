...
cc.Control = cc.Layer.extend({_isOpacityModifyRGB: false, _hasVisibleParents: false, _touchListener: null, _className: "Control", isOpacityModifyRGB: function () {
    ...
    _selected: false, _highlighted: false, _dispatchTable: null, setEnabled: function (enabled) {
...
cc.ControlButton = cc.Control.extend({_doesAdjustBackgroundImage: false, zoomOnTouchDown: false, _preferredSize: null, _labelAnchorPoint: null, _currentTitle: null, _currentTitleColor: null, _titleLabel: null, _backgroundSprite: null, _opacity: 0, _isPushed: false, _titleDispatchTable: null, _titleColorDispatchTable: null, _titleLabelDispatchTable: null, _backgroundSpriteDispatchTable: null, _parentInited: false, _marginV: 0, _marginH: 0, _className: "ControlButton", ctor: function (label, backgroundSprite, fontSize) {
    ...
    if (fontSize != undefined)this.initWithTitleAndFontNameAndFontSize(label, backgroundSprite, fontSize); else if (backgroundSprite != undefined)this.initWithLabelAndBackgroundSprite(label, backgroundSprite); else if (label != undefined)this.initWithBackgroundSprite(label);
    else this.init()
}, init: function () {
    ...
    if (locBackgroundSprite)locBackgroundSprite.setPosition(locContentSize.width / 2, locContentSize.height /
        2);
    ...
    if (this._doesAdjustBackgroundImage) {
        ...
        if (preferredSize.width <=
            0)preferredSize.width = titleLabelSize.width;
    ...
    if (label) {
        label.setPosition(locContentSize.width /
            2, locContentSize.height / 2);
    ...
    if (!label)throw"cc.ControlButton.initWithLabelAndBackgroundSprite(): label should be non-null";
    ...
    setMargins: function (marginH, marginV) {
        ...
        } else if (isTouchMoveInside && this._highlighted)this.sendActionsForControlEvents(cc.CONTROL_EVENT_TOUCH_DRAG_INSIDE); else if (!isTouchMoveInside && this._highlighted) {
...
cc.Scale9Sprite = cc.Node.extend({_spriteRect: null, _capInsetsInternal: null, _positionsAreDirty: false, _scale9Image: null, _topLeft: null, _top: null, _topRight: null, _left: null, _centre: null, _right: null, _bottomLeft: null, _bottom: null, _bottomRight: null, _opacityModifyRGB: false, _originalSize: null, _preferredSize: null, _opacity: 0, _color: null, _capInsets: null, _insetLeft: 0, _insetTop: 0, _insetRight: 0, _insetBottom: 0, _spritesGenerated: false, _spriteFrameRotated: false, _textureLoaded: false, _loadedEventListeners: null, _className: "Scale9Sprite",
    textureLoaded: function () {
        ...
        var insets, locInsetLeft = this._insetLeft,
            locInsetTop = this._insetTop, locInsetRight = this._insetRight;
        ...
        var locLeft = this._left,
            locRight = this._right, locTop = this._top, locBottom = this._bottom;
        var tempAP = cc.p(0, 0);
        ...
        locTopLeft.setPosition(0, bottomHeight +
            rescaledHeight);
        ...
        if (file != undefined)if (file instanceof cc.SpriteFrame)this.initWithSpriteFrame(file, rect); else {
    ...
    getPreferredSize: function () {
        ...
        for (var i = 0; i < scaleChildren.length; i++) {
            ...
            if (selChild)selChild.setOpacity(opacity)
        }
        ...
        for (var i = 0; i < scaleChildren.length; i++) {
            var selChild = scaleChildren[i];
            if (selChild)selChild.updateDisplayedOpacity(parentOpacity)
        }
    },
    setColor: function (color) {
        if (!this._scale9Image)return;
        ...
        for (var i = 0; i < scaleChildren.length; i++) {
            var selChild = scaleChildren[i];
            if (selChild)selChild.setColor(color)
        }
        ...
        for (var i = 0; i < scaleChildren.length; i++) {
            ...
            if (selChild) {
                ...
                if (cc._renderType === cc._RENDER_TYPE_CANVAS && (parentColor.r !== 255 || parentColor.g !== 255 || parentColor.b !== 255)) {
        ...
        this.updateWithBatchNode(this._scale9Image,
            this._spriteRect, this._spriteFrameRotated, capInsets);
    ...
    setInsetBottom: function (insetBottom) {
        ...
        if (batchNode)this.updateWithBatchNode(batchNode, rect, rotated, capInsets);
    ...
    isOpacityModifyRGB: function () {
        ...
        if (this._scale9Image != batchNode)this._scale9Image = batchNode;
        ...
        var top_h = locCapInsetsInternal.y,
            center_h = locCapInsetsInternal.height, bottom_h = h - (top_h + center_h);
        var x = 0, y = 0;
        ...
        if (!rotated) {
            ...
            cc._rectApplyAffineTransformIn(rightcenterbounds,
                t);
            ...
            this._bottom.initWithTexture(selTexture,
                centerbottombounds);
            ...
            locScale9Image.addChild(this._topLeft,
                2, cc.Scale9Sprite.POSITIONS_TOPLEFT);
            ...
            locScale9Image.addChild(this._bottomRight,
                2, cc.Scale9Sprite.POSITIONS_BOTTOMRIGHT)
        } else {
            ...
            centerbottombounds = cc.rectApplyAffineTransform(centerbottombounds,
                t);
        ...
        if (!locLoaded)spriteFrame.addLoadedEventListener(function (sender) {
            var preferredSize = this._preferredSize;
            preferredSize = cc.size(preferredSize.width, preferredSize.height);
            this.updateWithBatchNode(this._scale9Image, sender.getRect(), cc._renderType == cc._RENDER_TYPE_WEBGL &&
                sender.isRotated(), this._capInsets);
            this.setPreferredSize(preferredSize);
            this._positionsAreDirty = true;
            this._callLoadedEventCallbacks()
        }, this);
...
cc.MenuPassive = cc.Layer.extend({_color: null, _opacity: 0, _className: "MenuPassive", ctor: function () {
    ...
    locColor.g = color.g;
    locColor.b = color.b;
    if (this._children && this._children.length > 0)for (var i = 0; i < this._children.length; i++)if (this._children[i])this._children[i].setColor(color);
    ...
    getOpacity: function () {
        ...
        if (this._children && this._children.length > 0)for (var i = 0; i < this._children.length; i++)if (this._children[i])this._children[i].setOpacity(opacity);
        ...
        var height = -padding;
        var i;
        if (this._children && this._children.length > 0)for (i = 0; i < this._children.length; i++)if (this._children[i])height += this._children[i].getContentSize().height * this._children[i].getScaleY() + padding;
        var width = 0;
        var y =
            height / 2;
        ...
        if (this._children && this._children.length > 0)for (i = 0; i < this._children.length; i++)if (this._children[i]) {
            ...
            this._children[i].setPosition(x + this._children[i].getContentSize().width * this._children[i].getScaleX() / 2,
                0);
...
cc.ControlSaturationBrightnessPicker = cc.Control.extend({_saturation: 0, _brightness: 0, _background: null, _overlay: null, _shadow: null, _slider: null, _startPos: null, _boxPos: 0, _boxSize: 0, _className: "ControlSaturationBrightnessPicker", ctor: function (target, pos) {
    ...
    getShadow: function () {
        ...
        if (cc.Control.prototype.init.call(this)) {
            ...
            this._shadow = cc.ControlUtils.addSpriteToTargetWithPosAndAnchor("colourPickerShadow.png",
                target, pos, cc.p(0, 0));
        ...
        if (sliderPosition.x < this._startPos.x + this._boxPos)sliderPosition.x = this._startPos.x + this._boxPos; else if (sliderPosition.x > this._startPos.x + this._boxPos + this._boxSize - 1)sliderPosition.x = this._startPos.x + this._boxPos + this._boxSize -
            1;
        ...
        var centerX = this._startPos.x + this._background.getBoundingBox().width *
            0.5;
        var centerY = this._startPos.y + this._background.getBoundingBox().height * 0.5;
...
cc.ControlHuePicker = cc.Control.extend({_hue: 0, _huePercentage: 0, _background: null, _slider: null, _startPos: null, _className: "ControlHuePicker", ctor: function (target, pos) {
    ...
    cc.Control.prototype.setEnabled.call(this, enabled);
    if (this._slider)this._slider.setOpacity(enabled ?
        255 : 128)
}, getBackground: function () {
    return this._background
}, getSlider: function () {
    ...
    if (cc.Control.prototype.init.call(this)) {
        ...
        this._slider = cc.ControlUtils.addSpriteToTargetWithPosAndAnchor("colourPicker.png", target, pos, cc.p(0.5, 0.5));
        this._slider.setPosition(pos.x, pos.y + this._background.getBoundingBox().height *
            0.5);
        this._startPos = pos;
        this._hue = 0;
    ...
    var distance =
        Math.sqrt(Math.pow(location.x + 10, 2) + Math.pow(location.y, 2));
...
cc.ControlColourPicker = cc.Control.extend({_hsv: null, _colourPicker: null, _huePicker: null, _background: null, _className: "ControlColourPicker", ctor: function () {
    ...
    colourSliderValueChanged: function (sender, controlEvent) {
        ...
        if (cc.Control.prototype.init.call(this)) {
            ...
            var backgroundPointZero =
                cc.pSub(this._background.getPosition(), cc.p(this._background.getContentSize().width / 2, this._background.getContentSize().height / 2));
            var hueShift = 8;
            var colourShift = 28;
            ...
            this._huePicker.addTargetWithActionForControlEvents(this, this.hueSliderValueChanged,
...
cc.ControlSlider = cc.Control.extend({_value: 0, _minimumValue: 0, _maximumValue: 0, _minimumAllowedValue: 0, _maximumAllowedValue: 0, _thumbSprite: null, _progressSprite: null, _backgroundSprite: null, _className: "ControlSlider", ctor: function (bgFile, progressFile, thumbFile) {
    ...
    setMaximumValue: function (maximumValue) {
        ...
        return cc.rectContainsPoint(rect,
            touchLocation)
    }, locationFromTouch: function (touch) {
        var touchLocation = touch.getLocation();
        touchLocation = this.convertToNodeSpace(touchLocation);
        if (touchLocation.x < 0)touchLocation.x = 0; else if (touchLocation.x > this._backgroundSprite.getContentSize().width)touchLocation.x = this._backgroundSprite.getContentSize().width;
    ...
    setMaximumAllowedValue: function (val) {
        ...
        if (cc.Control.prototype.init.call(this)) {
            ...
            var maxRect =
                cc.ControlUtils.CCRectUnion(backgroundSprite.getBoundingBox(), thumbSprite.getBoundingBox());
            this.setContentSize(maxRect.width, maxRect.height);
        ...
        cc.Control.prototype.setEnabled.call(this, enabled);
        if (this._thumbSprite)this._thumbSprite.setOpacity(enabled ? 255 : 128)
...
cc.ControlSwitch = cc.Control.extend({_switchSprite: null, _initialTouchXPosition: 0, _moved: false, _on: false, _className: "ControlSwitch", ctor: function (maskSprite, onSprite, offSprite, thumbSprite, onLabel, offLabel) {
    ...
    if (this.init()) {
        ...
        this._switchSprite.setPosition(this._switchSprite.getContentSize().width /
            2, this._switchSprite.getContentSize().height / 2);
    ...
    if (animated)this._switchSprite.runAction(cc.ActionTween.create(0.2, "sliderXPosition", this._switchSprite.getSliderXPosition(),
        xPosition)); else this._switchSprite.setSliderXPosition(xPosition);
    this.sendActionsForControlEvents(cc.CONTROL_EVENT_VALUECHANGED)
}, isOn: function () {
    return this._on
}, hasMoved: function () {
    ...
    var location = this.locationFromTouch(touch);
    this._switchSprite.getThumbSprite().setColor(cc.color.WHITE);
    if (this.hasMoved())this.setOn(!(location.x < this._switchSprite.getContentSize().width / 2), true); else this.setOn(!this._on,
        true)
}});
...
cc.ControlSwitchSprite = cc.Sprite.extend({_sliderXPosition: 0, _onPosition: 0, _offPosition: 0, _textureLocation: 0, _maskLocation: 0, _maskSize: null, _onSprite: null, _offSprite: null, _thumbSprite: null, _onLabel: null, _offLabel: null, _clipper: null, _stencil: null, _backRT: null, ctor: function () {
    ...
    if (this._onLabel)this._onLabel.setPosition(this._onSprite.getPositionX() - this._thumbSprite.getContentSize().width /
        6, this._onSprite.getContentSize().height / 2 - maskSize.height / 2);
    if (this._offLabel)this._offLabel.setPosition(this._offSprite.getPositionX() + this._thumbSprite.getContentSize().width / 6, this._offSprite.getContentSize().height / 2 - maskSize.height / 2);
    ...
    if (sliderXPosition <= this._offPosition)sliderXPosition = this._offPosition; else if (sliderXPosition >= this._onPosition)sliderXPosition =
        this._onPosition;
...
cc.ControlStepper = cc.Control.extend({_minusSprite: null, _plusSprite: null, _minusLabel: null, _plusLabel: null, _value: 0, _continuous: false, _autorepeat: false, _wraps: false, _minimumValue: 0, _maximumValue: 0, _stepValue: 0, _touchInsideFlag: false, _touchedPart: cc.CONTROL_STEPPER_PARTNONE, _autorepeatCount: 0, _className: "ControlStepper", ctor: function (minusSprite, plusSprite) {
    ...
    if (this.init()) {
        ...
        this.setPlusLabel(cc.LabelTTF.create("+", cc.CONTROL_STEPPER_LABELFONT, 40,
            cc.size(40, 40), cc.TEXT_ALIGNMENT_CENTER, cc.VERTICAL_TEXT_ALIGNMENT_CENTER));
        ...
        this.setContentSize(this._minusSprite.getContentSize().width + this._plusSprite.getContentSize().height,
            maxRect.height);
        return true
    }
    return false
    ...
    if (stepValue <=
        0)throw"cc.ControlStepper.setMaximumValue(): stepValue should be numerically greater than 0.";
    ...
    this._value = value;
    if (!this._wraps) {
        this._minusLabel.setColor(value ==
            this._minimumValue ? cc.CONTROL_STEPPER_LABELCOLOR_DISABLED : cc.CONTROL_STEPPER_LABELCOLOR_ENABLED);
    ...
    onTouchBegan: function (touch, event) {
        ...
        if (this.isTouchInside(touch)) {
            var location = this.getTouchLocation(touch);
            this.setValue(this._value + (location.x < this._minusSprite.getContentSize().width ? 0 - this._stepValue :
                this._stepValue))
        }
...
cc.ControlPotentiometer = cc.Control.extend({_thumbSprite: null, _progressTimer: null, _previousLocation: null, _value: 0, _minimumValue: 0, _maximumValue: 1, _className: "ControlPotentiometer", ctor: function (backgroundFile, progressFile, thumbFile) {
    ...
    if (thumbFile != undefined) {
        ...
        this.initWithTrackSprite_ProgressTimer_ThumbSprite(backgroundSprite,
            progressTimer, thumbSprite)
    }
    ...
    isTouchInside: function (touch) {
        ...
        this.setValue(this._value +
            angle / 360 * (this._maximumValue - this._minimumValue));
...
cc.ScrollView = cc.Layer.extend({_zoomScale: 0, _minZoomScale: 0, _maxZoomScale: 0, _delegate: null, _direction: cc.SCROLLVIEW_DIRECTION_BOTH, _dragging: false, _contentOffset: null, _container: null, _touchMoved: false, _maxInset: null, _minInset: null, _bounceable: false, _clippingToBounds: false, _scrollDistance: null, _touchPoint: null, _touchLength: 0, _touches: null, _viewSize: null, _minScale: 0, _maxScale: 0, _parentScissorRect: null, _scissorRestored: false, _tmpViewRect: null, _touchListener: null, _className: "ScrollView", ctor: function (size, container) {
    ...
    getContentOffset: function () {
        ...
        return cc.p(locViewSize.width -
            locContentSize.width * locContainer.getScaleX(), locViewSize.height - locContentSize.height * locContainer.getScaleY())
    ...
    setDelegate: function (delegate) {
        ...
        if (this._touches.length ===
            ...
            if (locDirection === cc.SCROLLVIEW_DIRECTION_VERTICAL) {
                ...
                pos = this._container.getPositionX();
                if (!(_minOffset.x <= pos && pos <= _maxOffset.x))moveDistance.x *=
                    BOUNCE_BACK_FACTOR
            }
            if (!this._touchMoved && Math.abs(cc.convertDistanceFromPointToInch(dis)) < MOVE_INCH)return;
            if (!this._touchMoved) {
                moveDistance.x = 0;
                moveDistance.y = 0
            }
        ...
        if (this.getContainer() != null) {
            ...
            this._minInset.y =
                tempOffset.y - locViewSize.height * INSET_RATIO
        }
        ...
        if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
            ...
            if (locChildren && locChildren.length > 0) {
                ...
                for (i = 0; i < childrenLen; i++) {
                    ...
                    if (selChild && selChild._localZOrder < 0)selChild.visit(context); else break
                }
                this.draw(context);
            ...
            if (locChildren && locChildren.length > 0) {
                ...
                for (i = 0; i < childrenLen; i++) {
                    selChild = locChildren[i];
                    if (selChild && selChild._localZOrder < 0)selChild.visit(); else break
                }
                this.draw(context);
        ...
        if (this._container != child)this._container.addChild(child,
            zOrder, tag); else cc.Layer.prototype.addChild.call(this, child, zOrder, tag)
        ...
        if (!e) {
            ...
            var listener = cc.EventListener.create({event: cc.EventListener.TOUCH_ONE_BY_ONE});
            if (this.onTouchBegan)listener.onTouchBegan = this.onTouchBegan.bind(this);
            if (this.onTouchMoved)listener.onTouchMoved =
                this.onTouchMoved.bind(this);
            if (this.onTouchEnded)listener.onTouchEnded = this.onTouchEnded.bind(this);
            if (this.onTouchCancelled)listener.onTouchCancelled = this.onTouchCancelled.bind(this);
            this._touchListener = listener;
        ...
        locScrollDistance.y =
            locScrollDistance.y * SCROLL_DEACCEL_RATE;
        this.setContentOffset(cc.p(newX, newY));
...
cc.TableView = cc.ScrollView.extend({_vOrdering: null, _indices: null, _cellsFreed: null, _dataSource: null, _tableViewDelegate: null, _oldDirection: null, _cellsPositions: null, _touchedCell: null, ctor: function (dataSource, size, container) {
    ...
    switch (this.getDirection()) {
        case cc.SCROLLVIEW_DIRECTION_HORIZONTAL:
            offset = cc.p(this._cellsPositions[index],
                0);
            break;
        default:
    ...
    if (cellsCount > 0) {
        var maxPosition =
            this._cellsPositions[cellsCount];
        switch (this.getDirection()) {
            case cc.SCROLLVIEW_DIRECTION_HORIZONTAL:
    ...
    if (this._tableViewDelegate &&
        this._tableViewDelegate.tableCellWillRecycle)this._tableViewDelegate.tableCellWillRecycle(this, cell);
    ...
    if (cell.getParent() !=
        this.getContainer())this.getContainer().addChild(cell);
    ...
    if (this._vOrdering !=
        ...
        if (this._cellsUsed.count() > 0)this.reloadData()
    }
    ...
    if (cc.ScrollView.prototype.initWithViewSize.call(this, size, container)) {
        ...
        cc.ScrollView.prototype.setDelegate.call(this,
            this);
        return true
    }
    return false
    ...
    var cell =
        locCellsUsed.objectWithObjectID(idx);
    if (cell) {
    ...
    this._oldDirection = cc.SCROLLVIEW_DIRECTION_NONE;
    var locCellsUsed = this._cellsUsed, locCellsFreed = this._cellsFreed, locContainer =
        this.getContainer();
    ...
    this._updateCellPositions();
    this._updateContentSize();
    if (this._dataSource.numberOfCellsInTableView(this) >
        0)this.scrollViewDidScroll(this)
}, dequeueCell: function () {
    if (this._cellsFreed.count() === 0)return null; else {
    ...
    if (this._tableViewDelegate !=
        null && this._tableViewDelegate.scrollViewDidScroll)this._tableViewDelegate.scrollViewDidScroll(this);
    ...
    if (this._vOrdering === cc.TABLEVIEW_FILL_TOPDOWN)offset.y -=
        locViewSize.height / locContainer.getScaleY(); else offset.y += locViewSize.height / locContainer.getScaleY();
    ...
    cc.ScrollView.prototype.onTouchEnded.call(this,
        touch, event)
}, onTouchBegan: function (touch, event) {
    if (!this.isVisible())return false;
    ...
    if (this._touches.length === 1) {
        ...
        if (this._touchedCell && this._tableViewDelegate != null && this._tableViewDelegate.tableCellHighlight)this._tableViewDelegate.tableCellHighlight(this,
            this._touchedCell)
    ...
    if (this._touchedCell && this.isTouchMoved()) {
        if (this._tableViewDelegate != null && this._tableViewDelegate.tableCellUnhighlight)this._tableViewDelegate.tableCellUnhighlight(this,
            this._touchedCell);
        this._touchedCell = null
    }
}, onTouchCancelled: function (touch, event) {
