...
(function () {
    ...
    if (navigator.userAgent.indexOf("MicroMessenger") > -1)win.onfocus = function () {
        onShow()
    };
...
cc._initSys = function (config, CONFIG_KEY) {
    ...
    sys.isNative = false;
    var webglWhiteList = [sys.BROWSER_TYPE_BAIDU, sys.BROWSER_TYPE_OPERA, sys.BROWSER_TYPE_FIREFOX, sys.BROWSER_TYPE_CHROME,
        sys.BROWSER_TYPE_SAFARI];
    var multipleAudioWhiteList = [sys.BROWSER_TYPE_BAIDU, sys.BROWSER_TYPE_OPERA, sys.BROWSER_TYPE_FIREFOX, sys.BROWSER_TYPE_CHROME, sys.BROWSER_TYPE_SAFARI, sys.BROWSER_TYPE_UC, sys.BROWSER_TYPE_QQ, sys.BROWSER_TYPE_MOBILE_QQ, sys.BROWSER_TYPE_IE];
    ...
    var browserTypes = ua.match(/micromessenger|qqbrowser|mqqbrowser|ucbrowser|360browser|baiduboxapp|baidubrowser|maxthon|trident|opera|miuibrowser|firefox/i) || ua.match(/chrome|safari/i);
    if (browserTypes && browserTypes.length > 0) {
        browserType = browserTypes[0].toLowerCase();
        if (browserType == "micromessenger")browserType =
            sys.BROWSER_TYPE_WECHAT; else if (browserType === "safari" && ua.match(/android.*applewebkit/))browserType = sys.BROWSER_TYPE_ANDROID; else if (browserType == "trident")browserType = sys.BROWSER_TYPE_IE
    }
    ...
    sys._supportCanvasNewBlendModes =
        sys._canUseCanvasNewBlendModes();
    ...
    if (docEle["ontouchstart"] !== undefined || nav.msPointerEnabled)capabilities["touches"] =
        true; else if (docEle["onmouseup"] !== undefined)capabilities["mouse"] = true;
...
cc._setup = function (el, width, height) {
    ...
    var frameTime = 1E3 / cc.game.config[cc.game.CONFIG_KEY.frameRate];
    win.requestAnimFrame = win.requestAnimationFrame || win.webkitRequestAnimationFrame || win.mozRequestAnimationFrame || win.oRequestAnimationFrame || win.msRequestAnimationFrame || function (callback) {
...
cc.game = {DEBUG_MODE_NONE: 0, DEBUG_MODE_INFO: 1, DEBUG_MODE_WARN: 2, DEBUG_MODE_ERROR: 3, DEBUG_MODE_INFO_FOR_WEB_PAGE: 4, DEBUG_MODE_WARN_FOR_WEB_PAGE: 5, DEBUG_MODE_ERROR_FOR_WEB_PAGE: 6, EVENT_HIDE: "game_on_hide", EVENT_SHOW: "game_on_show", _eventHide: null, _eventShow: null, _onBeforeStartArr: [], CONFIG_KEY: {engineDir: "engineDir", dependencies: "dependencies", debugMode: "debugMode", showFPS: "showFPS", frameRate: "frameRate", id: "id", renderMode: "renderMode", jsList: "jsList", classReleaseMode: "classReleaseMode"}, _prepareCalled: false,
    _prepared: false, _paused: true, _intervalId: null, config: null, onStart: null, onStop: null, setFrameRate: function (frameRate) {
        ...
        document.body ? _run() : cc._addEventListener(window,
            "load", function () {
                this.removeEventListener("load", arguments.callee, false);
                _run()
            }, false)
        ...
        if (document["ccConfig"])self.config = _init(document["ccConfig"]);
        else try {
        ...
        for (var i = 0, li = tempList.length; i < li; i++) {
            ...
            if (!extname) {
                var arr = this._getJsListOfModule(moduleMap,
                    item, dir);
...
cc.checkGLErrorDebug = function () {
    if (cc.renderMode == cc._RENDER_TYPE_WEBGL) {
        ...
        if (_error)cc.log(cc._LogInfos.checkGLErrorDebug, _error)
    }
};
...
cc._tmp.PrototypeColor = function () {
    ...
    cc.BlendFunc._disable = function () {
        return new cc.BlendFunc(cc.ONE,
            cc.ZERO)
    };
...
cc.Node = cc.Class.extend({_localZOrder: 0, _globalZOrder: 0, _vertexZ: 0, _rotationX: 0, _rotationY: 0, _scaleX: 1, _scaleY: 1, _position: null, _skewX: 0, _skewY: 0, _children: null, _visible: true, _anchorPoint: null, _anchorPointInPoints: null, _contentSize: null, _running: false, _parent: null, _ignoreAnchorPointForPosition: false, tag: cc.NODE_TAG_INVALID, userData: null, userObject: null, _transformDirty: true, _inverseDirty: true, _cacheDirty: true, _cachedParent: null, _transformGLDirty: null, _transform: null, _inverse: null, _reorderChildDirty: false,
    ...
    ignoreAnchorPointForPosition: function (newValue) {
        ...
        if (this.arrivalOrder == NaN)debugger;
    ...
    getNodeToWorldTransform: function () {
        ...
        for (var p = this._parent; p != null; p = p.parent)t = cc.affineTransformConcat(t, p.getNodeToParentTransform());
        return t
        ...
        return cc.pointApplyAffineTransform(worldPoint,
        ...
        var worldPoint =
            this.convertToWorldSpace(nodePoint);
    ...
    getGrid: function () {
        ...
        rect = cc.rectApplyAffineTransform(rect,
        ...
        var rect = cc.rect(0, 0, this._contentSize.width, this._contentSize.height);
        var trans = parentTransform == null ? this.getNodeToParentTransform() : cc.affineTransformConcat(this.getNodeToParentTransform(),
            parentTransform);
        ...
        if (_t._transformDirty) {
            ...
            var apy =
                _t._anchorPointInPoints.y, napy = -apy;
...
cc.Sprite = cc.Node.extend({dirty: false, atlasIndex: 0, textureAtlas: null, _batchNode: null, _recursiveDirty: null, _hasChildren: null, _shouldBeHidden: false, _transformToBatch: null, _blendFunc: null, _texture: null, _rect: null, _rectRotated: false, _offsetPosition: null, _unflippedOffsetPositionFromCenter: null, _opacityModifyRGB: false, _flippedX: false, _flippedY: false, _textureLoaded: false, _loadedEventListeners: null, _newTextureWhenChangeColor: null, _className: "Sprite", _oldDisplayColor: cc.color.WHITE, textureLoaded: function () {
    ...
    setVertexRect: function (rect) {
        ...
        if (!norecursive && this._batchNode && !this._recursiveDirty)if (this._hasChildren)this.setDirtyRecursively(true);
        else {
    ...
    }, initWithTexture: null, _textureLoadedCallback: null,
    setTextureRect: null, updateTransform: null, addChild: null, updateColor: function () {
        ...
        return cc.SpriteFrame.create(this._texture, cc.rectPointsToPixels(this._rect), this._rectRotated,
            cc.pointPointsToPixels(this._unflippedOffsetPositionFromCenter), cc.sizePointsToPixels(this._contentSize))
    ...
    _changeTextureColor: function () {
        ...
        if (locTexture && locRect.validRect && this._originalTexture) {
            ...
            if (locElement instanceof HTMLCanvasElement && !this._rectRotated && !this._newTextureWhenChangeColor)cc.generateTintImageWithMultiply(this._originalTexture._htmlElementObj, this._displayedColor, locRect, locElement); else {
                locElement = cc.generateTintImageWithMultiply(this._originalTexture._htmlElementObj,
                    this._displayedColor, locRect);
...
if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
    ...
    _p.updateDisplayedColor = function (parentColor) {
        ...
        if (oColor.r === nColor.r && oColor.g === nColor.g && oColor.b === nColor.b)return;
...
cc.LabelTTF = cc.Sprite.extend({_dimensions: null, _hAlignment: cc.TEXT_ALIGNMENT_CENTER, _vAlignment: cc.VERTICAL_TEXT_ALIGNMENT_TOP, _fontName: null, _fontSize: 0, _string: "", _originalText: null, _isMultiLine: false, _fontStyleStr: null, _shadowEnabled: false, _shadowOffset: null, _shadowOpacity: 0, _shadowBlur: 0, _shadowColorStr: null, _strokeEnabled: false, _strokeColor: null, _strokeSize: 0, _strokeColorStr: null, _textFillColor: null, _fillColorStr: null, _strokeShadowOffsetX: 0, _strokeShadowOffsetY: 0, _needUpdateTexture: false, _labelCanvas: null,
    ...
    }, setColor: null, _setColorsString: null, updateDisplayedColor: null, setOpacity: null, updateDisplayedOpacity: null,
    ...
    _setShadowBlur: function (shadowBlur) {
        ...
        if (locStrokeColor.r !== strokeColor.r || locStrokeColor.g !== strokeColor.g || locStrokeColor.b !==
            strokeColor.b) {
    ...
    _updateWithTextDefinition: function (textDefinition, mustUpdateTexture) {
        ...
        if (allWidth > maxWidth && text.length > 1) {
            ...
            if (cc.LabelTTF.wrapInspection)if (cc.LabelTTF._symbolRex.test(sLine ||
                tmpText)) {
        ...
        if (locDimensionsWidth !== 0) {
            ...
            for (i = 0; i < this._strings.length; i++)this._checkWarp(this._strings, i, locDimensionsWidth)
        } else {
            this._strings = this._string.split("\n");
        ...
        if (this._strokeEnabled)locStrokeShadowOffsetX =
            locStrokeShadowOffsetY = this._strokeSize * 2;
        if (this._shadowEnabled) {
            var locOffsetSize = this._shadowOffset;
            locStrokeShadowOffsetX += Math.abs(locOffsetSize.x) * 2;
            locStrokeShadowOffsetY += Math.abs(locOffsetSize.y) * 2
        }
        if (locDimensionsWidth === 0)if (this._isMultiLine)locSize = cc.size(0 | Math.max.apply(Math, locLineWidth) + locStrokeShadowOffsetX, 0 | this._fontClientHeight * this._strings.length + locStrokeShadowOffsetY); else locSize = cc.size(0 | this._measure(this._string) + locStrokeShadowOffsetX, 0 | this._fontClientHeight + locStrokeShadowOffsetY);
        else if (this._dimensions.height === 0)if (this._isMultiLine)locSize = cc.size(0 | locDimensionsWidth + locStrokeShadowOffsetX, 0 | this._fontClientHeight * this._strings.length + locStrokeShadowOffsetY); else locSize = cc.size(0 | locDimensionsWidth + locStrokeShadowOffsetX, 0 | this._fontClientHeight + locStrokeShadowOffsetY); else locSize = cc.size(0 | locDimensionsWidth + locStrokeShadowOffsetX, 0 | this._dimensions.height + locStrokeShadowOffsetY);
        this.setContentSize(locSize);
        this._strokeShadowOffsetX = locStrokeShadowOffsetX;
        this._strokeShadowOffsetY =
