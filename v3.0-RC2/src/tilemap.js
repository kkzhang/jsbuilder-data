...
cc.TMXLayer = cc.SpriteBatchNode.extend({tiles: null, tileset: null, layerOrientation: null, properties: null, layerName: "", _layerSize: null, _mapTileSize: null, _opacity: 255, _minGID: null, _maxGID: null, _vertexZvalue: null, _useAutomaticVertexZ: null, _alphaFuncValue: null, _reusedTile: null, _atlasIndexArray: null, _contentScaleFactor: null, _cacheCanvas: null, _cacheContext: null, _cacheTexture: null, _subCacheCanvas: null, _subCacheContext: null, _subCacheCount: 0, _subCacheWidth: 0, _maxCachePixel: 1E7, _className: "TMXLayer", ctor: function (tilesetInfo, layerInfo, mapInfo) {
    ...
    if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
        ...
        if (this.layerOrientation === cc.TMX_ORIENTATION_HEX)this._cacheContext.translate(0, locCanvas.height - this._mapTileSize.height * 0.5); else this._cacheContext.translate(0, locCanvas.height);
    ...
    if (this._cacheDirty) {
        ...
        locCacheContext.clearRect(0, 0, locCacheCanvas.width,
            -locCacheCanvas.height);
        ...
        if (this._subCacheCount > 0) {
            ...
            for (i = 0; i < this._subCacheCount; i++)this._subCacheContext[i].drawImage(locCacheCanvas, i * subCacheW, 0, subCacheW, subCacheH, 0, 0, subCacheW,
                subCacheH)
        }
    ...
    removeChild: function (sprite, cleanup) {
        ...
        return cc.p(pos.x * this._mapTileSize.width * 3 / 4, (this._layerSize.height - pos.y - 1) * this._mapTileSize.height +
            diffY)
        ...
        rect = cc.rect(rect.x /
            locScaleFactor, rect.y / locScaleFactor, rect.width / locScaleFactor, rect.height / locScaleFactor);
        var z = pos.x + pos.y * this._layerSize.width;
        var tile = this._reusedTileWithRect(rect);
        this._setupTileSprite(tile, pos, gid);
        ...
        if ((gid & cc.TMX_TILE_DIAGONAL_FLAG) >>> 0) {
            ...
            sprite.y =
                this.getPositionAt(pos).y + sprite.height / 2;
            ...
            if ((gid & cc.TMX_TILE_VERTICAL_FLAG) >>>
                0)sprite.setFlippedY(true)
        }
