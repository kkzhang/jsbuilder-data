...
(function (P) {
    var name = "facebook";
    var userInfo = {};
    var errMsg = {"0": "success", 1: "Unknown error", 2: "Network error", 3: "Without permission", 4: "Interrupt operation"};
    var configCache = {};
    var isInit = false;
    P.extend(name, {init: function (config) {
        if (!FB || isInit)return;
        var self = this;
        ...
        FB.login(function (response) {
            ...
            } else typeof callback === "function" && callback(1, errMsg[1])
        }, {scope: ""})
        ...
        FB.logout(function (response) {
            if (response.authResponse) {
                ...
                userInfo = {};
                typeof callback === "function" && callback(0, errMsg[0])
            } else typeof callback ===
            "function" && callback(1, errMsg[1])
        })
    ...
    }}, share: {share: function (info, callback) {
        FB.ui({method: "share",
            name: info["title"], caption: info["caption"], description: info["text"], href: info["link"], picture: info["imageUrl"]}, function (response) {
            if (response)if (response.post_id)typeof callback === "function" && callback(0, errMsg[0]); else typeof callback === "function" && callback(3, errMsg[3]); else typeof callback === "function" && callback(4, errMsg[4])
        })
    }}, social: {submitScore: function (callback) {
        if (userInfo.userID)FB.api("/" + userInfo.userID + "/scores", "post", {score: 100}, function (response) {
            console.log(response);
            if (response)typeof callback ===
            "function" && callback(0, errMsg[0]); else typeof callback === "function" && callback(1, errMsg[1])
        }); else callback(3, errMsg[3])
    }, showLeaderboard: function (callback) {
        if (configCache["appId"])FB.api("/" + configCache["appId"] + "/scores", function (response) {
            if (response["error"])typeof callback === "function" && callback(1, response["message"]); else if (response["data"])typeof callback === "function" && callback(0, response["data"])
        })
        ...
        if (options["method"] == "share_open_graph" && options["url"])if (options["url"])options["action_properties"] = JSON.stringify({object: options["url"]}); else return; else if (!options["href"])return;
        FB.ui(options, function (response) {
            if (response)if (response.post_id)typeof callback === "function" && callback(0, errMsg[0]); else typeof callback ===
            "function" && callback(3, errMsg[3]); else typeof callback === "function" && callback(4, errMsg[4])
        })
    }, ui: FB.ui, api: FB.api})
})(plugin);