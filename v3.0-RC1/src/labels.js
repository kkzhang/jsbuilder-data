cc.LabelAtlas = cc.AtlasNode.extend({_string: null, _mapStartChar: null, _textureLoaded: false, _loadedEventListeners: null, _className: "LabelAtlas", ctor: function (strText, charMapFile, itemWidth, itemHeight, startCharMap) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        var rect = cc.rect(row * locItemWidth, col * locItemHeight,
            locItemWidth, locItemHeight);
        ...
        fontChar.setPosition(i *
            locItemWidth + locItemWidth / 2, locItemHeight / 2)
    }
    ...
    if (n >
        locTextureAtlas.getCapacity())cc.log("cc.LabelAtlas._updateAtlasValues(): Invalid String length");
    ...
    for (var i = 0; i < n; i++) {
        ...
        if (cc.FIX_ARTIFACTS_BY_STRECHING_TEXEL) {
            left =
                (2 * row * itemWidthInPixels + 1) / (2 * textureWide);
...
cc.LabelBMFont = cc.SpriteBatchNode.extend({_opacityModifyRGB: false, _string: "", _config: null, _fntFile: "", _initialString: "", _alignment: cc.TEXT_ALIGNMENT_CENTER, _width: -1, _lineBreakWithoutSpaces: false, _imageOffset: null, _reusedChar: null, _displayedOpacity: 255, _realOpacity: 255, _displayedColor: null, _realColor: null, _cascadeColorEnabled: true, _cascadeOpacityEnabled: true, _textureLoaded: false, _loadedEventListeners: null, _className: "LabelBMFont", _setString: function (newString, needUpdateLabel) {
    ...
    if (this._textureLoaded)if (this._cascadeColorEnabled) {
        ...
        if (locParent && locParent.cascadeColor)parentColor = locParent.getDisplayedColor();
    ...
    for (var i = 0; i < locChildren.length; i++) {
        ...
        if (cc._renderType ==
            cc._RENDER_TYPE_WEBGL)locChild.updateDisplayedOpacity(this._displayedOpacity); else {
    ...
    getDisplayedColor: function () {
        ...
        for (var i = 0; i < locChildren.length; i++) {
            var locChild = locChildren[i];
            if (cc._renderType == cc._RENDER_TYPE_WEBGL)locChild.updateDisplayedColor(this._displayedColor);
            else {
    ...
    init: function () {
        ...
        if (self.initWithTexture(texture, theString.length)) {
            ...
            if (cc._renderType === cc._RENDER_TYPE_WEBGL) {
                ...
                var reusedChar =
                    self._reusedChar = new cc.Sprite;
        ...
        for (i = 0; i < stringLen; i++) {
            ...
            var kerningAmount = locKerningDict[prev <<
                16 | key & 65535] || 0;
...
if (cc._renderType === cc._RENDER_TYPE_CANVAS && !cc.sys._supportCanvasNewBlendModes)_p._changeTextureColor = function () {
    if (cc._renderType == cc._RENDER_TYPE_WEBGL)return;
    ...
    if (locTexture && locTexture.getContentSize().width > 0) {
        ...
        if (cacheTextureForColor)if (locElement instanceof HTMLCanvasElement && !this._rectRotated)cc.generateTintImage(locElement,
            cacheTextureForColor, this._displayedColor, null, locElement); else {
